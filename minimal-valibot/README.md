...
by valibot. One module, one controller, one provider — wired at build time, with no decorators and no
runtime container.
...
| `src/users/user.schema.ts`      | The valibot schema. One declaration is the type and the validator. |
